...
# from storeCommerce.models import db_Profile
...
def signUp(request):
    ...
    try:
            ...
            db_country = var_country, db_date_DoB = var_date_DoB
            )
...
def verify(request , auth_token):
    try:
        ...
        var_bio = "N/A"
        var_id = profile_obj.id
        var_email = profile_obj.char_email
        ...
        if profile_obj:
            ...
            db_Profile_Det = db_Profile_detail.objects.create(db_email_id = var_id, 
                db_email = profile_obj,
                char_email = var_email,
                db_bio = var_bio,
                db_photo = var_photo
                )
            db_Profile_Det.save()
...
def send_mail_after_registration(email , token):
    subject = 'PinStore : Activation Code'
    domain = settings.LOCAL_HOST_STRIPE_Auth
    message = f'Welcome to PinStore !! \n Hi {email}, \n Please confirm your email by clicking on the following link.\n \n Confirmation Link: {domain}/verify/{token}'
